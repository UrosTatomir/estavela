...
// require_once"talas.php";
  ...
  Class Hidrogram
  {
	 ...
	 public function SCS($name)
		{
           echo'<h3 style="color:#06342D;">Metoda Talas SCS naziv sliva = '.$name.'</h3> <br><br>';
	  ...
	  public function maxO($Tk,$F,$k,$a,$L,$Lc,$Iu) //max Ordinata-----//$q_max = (0.56 * $A)/$Tb;
		  {
		  	echo'Vršna (max.)ordinata sintetičkog jediničnog hidrograma q_max [m3/smm]<br>';
			  	$result = [];
			  	foreach($Tk as $value)
			  	{
			  	 array_push($result,$value/$a);
	   ...
	   public function maxP($Tk,$F,$Ap,$k,$a,$b,$c,$L,$Lc,$Iu,$H24h,$Bm,$CN) //max proticaj----// $Qmax = $q_max * $Pe;
	   {	
	...
	public function connect($username,$password,$name,$a,$b,$c,$k,$L,$Lc,$Iu,$Ap,$F,$Bm,$H24h,$CN,$r)

	{
	
   $conn = new PDO("mysql:host=localhost;dbname=estavela_talas_db",$username,$password);  
      try
      {
      	$sql ="INSERT INTO scs(name,a,b,c,k,L,Lc,Iu,Ap,F,Bm,H24h,CN,Qmax)
       VALUES('$name','$a','$b','$c','$k','$L','$Lc','$Iu','$Ap','$F','$Bm','$H24h','$CN','$r[0]')";
       
       $conn->exec($sql);
       echo'<br>';	
       echo'New record insert successfully in db';
       echo'<br>';
